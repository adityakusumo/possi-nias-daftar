...
class NiasController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Query untuk data yang BELUM dikirim
        ...
        // Query untuk data yang SUDAH dikirim
        $sentRecords = Nias::where('user_id', Auth::id())
        ->where('is_sent', true)
    ...
    public function store(Request $request)
    {
        ...
        $validated = $request->validate([
            ...
            'NAMAKOTADOM'    => ($isUpdate ? 'required_if:tipe_update,update_domisili,update_all' : 'required') . '|nullable|string|max:100',
                                        'JENISDOM'       => 'nullable|string|max:10', // diambil dari domisiliLookup, tidak perlu required dari form
    ...
    public function export()
    {
        ...
        foreach ($allRecords as $i => $r) {
            ...
            foreach (['file_kk' => 'KK', 'file_foto' => 'Foto', 'file_akte' => 'Akte', 'file_ijazah' => 'Ijazah'] as $col => $label) {
                ...
                if (!Storage::disk('local')->exists($r->$col)) continue;
    ...
    public function sendEmail(Request $request)
    {
        ...
        $writeCsv = function ($records, string $label): string {
            ...
            fputcsv($out, ['NO','Club','NAMA LENGKAP ATLET','SUB CABANG OLAHRAGA','EMAIL',
                    'DOMISILI [SESUAI KK/KTP]','','','GENDER [Pa/Pi]','TEMPAT LAHIR',
                    'TGL LAHIR','NIK','STATUS NIAS [BARU / UPDATE]','NO. NIAS JATIM (UPDATE)',
                    'Daftar NIAS','Keterangan'], ';');
            fputcsv($out, ['','','','','','[PROVINSI]','[KOTA/KAB]','NAMA KOTA/KAB',
                    '','','','','','','',''], ';');
            foreach ($records as $i => $r) {
                fputcsv($out, [
                    $i + 1, $r->NAMACLUB, $r->NAMA, 'Finswimming', $r->EMAIL ?? '',
                    ($r->mutasi_luar_jatim === 'ya') ? '' : 'Jawa Timur',
                        $r->JENISDOM ?? '', $r->NAMAKOTADOM ?? '',
                        $r->GENDER === 'L' ? 'Pa' : 'Pi', $r->TEMPATLAHIR,
                        $r->TGLLAHIR?->format('d/m/Y') ?? '', $r->NIK ?? '',
                        $r->is_update ? 'UPDATE' : 'BARU', $r->NONIAS ?? '', 'JTM', '',
                ], ';');
            }
            fclose($out);
            return $tmp;
        ...
        foreach ($allRecords as $i => $r) {
            $folderAtlet = ($i + 1) . '_' . preg_replace('/[^A-Za-z0-9_\-]/', '_', $r->NAMA);
            $fileCols    = ['file_kk'=>'KK','file_foto'=>'Foto','file_akte'=>'Akte',
            'file_ijazah'=>'Ijazah','file_sk_mutasi'=>'SKMutasi'];
            foreach ($fileCols as $col => $label) {
                if (!$r->$col || !Storage::disk('local')->exists($r->$col)) continue;
                $storagePath = Storage::disk('local')->path($r->$col);
                $ext         = pathinfo($storagePath, PATHINFO_EXTENSION);
                $namaFile    = $label . '_' . preg_replace('/[^A-Za-z0-9_\-]/', '_', $r->NAMA) . '.' . $ext;
                $zip->addFile($storagePath, 'dokumen/' . $folderAtlet . '/' . $namaFile);
    ...
    public function showUpdateForm()
    {
        ...
        return view('nias.update_nias', compact('domisilis', 'userClub', 'expiredDate'));
    }
}
